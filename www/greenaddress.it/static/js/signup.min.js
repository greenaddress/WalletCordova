...
return ['$scope', '$location', 'mnemonics', 'tx_sender', 'notices', 'wallets', '$window', 'facebook', '$modal', 'gaEvent', '$q', 'reddit', 'storage', 'trezor', 'btchip', 'bip38', '$interval',
        function SignupController($scope, $location, mnemonics, tx_sender, notices, wallets, $window, facebook, $modal, gaEvent, $q, reddit, storage, trezor, btchip, bip38, $interval) {
    ...
    $scope.signup.usb_hwseed_modal = function() {
        btchip.setupSeed().then(function(result) {
            delete $scope.wallet.mnemonic;
            $scope.signup.mnemonic = gettext('Mnemonic not available when using BTChip seed');
            $scope.signup.has_btchip = true;
