...
public class NfcPlugin extends CordovaPlugin implements NfcAdapter.OnNdefPushCompleteCallback {
    ...
    public boolean execute(String action, JSONArray data, CallbackContext callbackContext) throws JSONException {
        ...
        } else if (action.equalsIgnoreCase(REMOVE_TAG)) {
            removeTag(data, callbackContext);
    ...
    private void removeMimeType(JSONArray data, CallbackContext callbackContext) throws JSONException {
        ...
        while (iter.hasNext()) {
            if (iter.next().hasDataType(mimeType)) {
                iter.remove();
                callbackContext.success();
                return;
            }
        }
        callbackContext.error("MIME Type not found " + mimeType);
    }
    ...
    private void removeTag(JSONArray data, CallbackContext callbackContext) throws JSONException {
        Iterator<IntentFilter> iter = intentFilters.iterator();
        while (iter.hasNext()) {
            if (iter.next().hasAction(NfcAdapter.ACTION_TAG_DISCOVERED)) {
                iter.remove();
                callbackContext.success();
                return;
            }
        }
        callbackContext.error("Tag filter not found");
    }
